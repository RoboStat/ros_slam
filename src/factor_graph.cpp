...
void FactorGraph::getPose(int poseID, cv::Mat& R, cv::Mat& T) {
	R.create(3,3,cv::DataType<double>::type);
	T.create(3,1,cv::DataType<double>::type);
	...
	T.at<double>(0, 0) = m(0, 3);
	T.at<double>(1, 0) = m(1, 3);
	T.at<double>(2, 0) = m(2, 3);
}

void FactorGraph::addLandMark(int landmarkID, const cv::Point3d& location) {
...
void FactorGraph::getLandMark(int landmarkID, cv::Point3d& location) {
...
void FactorGraph::addProjection(int poseID, int landmarkID, const cv::Point2d& location){
