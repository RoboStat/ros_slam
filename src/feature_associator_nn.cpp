...
FeatureAssociatorNN::FeatureAssociatorNN() :
				...
				stereoThre(50), stereoRatio(0.76),
				frameNum(0),
				filter(376,240){
...
bool FeatureAssociatorNN::trackLandmark(Landmark& landmark, const NNFinder& nnFinder) {
	using namespace cv;
...
bool FeatureAssociatorNN::pairLandmark(Landmark& landmark, const NNFinder& nnFinder) {
	using namespace cv;
	// compute nearest neighbour
	vector<int> boundPtsInd;
	...
	// determine whether a good match
	if (!matches.empty() && matches[0][0].distance < stereoThre &&
				(matches[0].size() == 1 ||
						(matches[0].size() == 2 &&
